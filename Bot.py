...
async def cmd_text(interaction: discord.Interaction, prompt: str, system: str = "You are a helpful assistant."):
    if not interaction.guild:
        await interaction.response.send_message(
            embed=discord.Embed(title="❌ Server only", description="Use this command in a server channel, not in DMs.", color=0xED4245),
            ephemeral=True
        )
        return
    ...
    try:
        ...
        # Manda messaggio nel canale e crea thread da esso
        ...
        # Poi manda il messaggio nel canale direttamente
        msg = await channel.send(embed=embed_intro)

        # Crea il thread dal messaggio del canale
        thread = await msg.create_thread(
            name=f"NovAI - {interaction.user.display_name} - {prompt[:40]}",
            auto_archive_duration=60
        )

        # Manda la risposta nel thread
        embed_reply = discord.Embed(description=reply[:4000], color=BOT_COLOR)
        embed_reply.set_footer(text="Just type here to keep chatting - type /close to end")
